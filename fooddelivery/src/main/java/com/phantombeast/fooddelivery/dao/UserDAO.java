...
public class UserDAO {
	...
	private static final String UPDATE_USER_SQL = "Update users Set name = ?, address = ?, mobile = ? where email = ?";
	...
	public SignupBean selectUser(String email) {
		...
		try {
			...
			while (rs.next()) {
				...
				user = new SignupBean(name, email, password, password, address, mobile);
			}
	...
	public boolean updateUser(SignupBean sb) {
		boolean success = false;

		PreparedStatement ps;
		try {
			ps = cn.prepareStatement(UPDATE_USER_SQL);
			ps.setString(1, sb.getName());
	...
	public boolean updatePassword(SignupBean sb) {
		boolean success = false;

		PreparedStatement ps;
		try {
			ps = cn.prepareStatement(UPDATE_PASSWORD_SQL);
			ps.setString(1, sb.getPassword());
			ps.setString(2, sb.getEmail());
