...
<body>
	...
	<div class="container">
		...
		<form method="post" action="settleOrder">
			<div>
				<%
				if (order.getStatus().toString() == "PENDING") {
				%>
				<input type="hidden" value="<%=order.getId()%>" name="id"> <select
					name="status" required>
					<option value="DELIVERED">DELIVERED</option>
					<option value="REJECTED">REJECTED</option>
				</select>
				<button type="submit" class="btn btn-primary">Settle Order</button>
				<%}%>
			</div>
		</form>
	</div>
